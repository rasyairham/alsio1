...
const Dashboardpage = () => {
  ...
  const refreshAllData = useCallback(async () => {
    try {
      // PERBAIKAN: Tidak perlu lagi set header manual & full URL
  ...
  const handleAddTask = async e => {
    e.preventDefault();
    try {
      // PERBAIKAN: Menggunakan instance api dan relative path
      ...
      alert(err.response?.data?.message || "Failed to deploy mission."); 
    }
  };
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8F5F2]">
      <div className="h-12 w-12 md:h-16 md:w-16 border-4 md:border-8 border-zinc-200 border-t-[#C29976] rounded-full animate-spin"></div>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-2 bg-white rounded-[2.5rem] md:rounded-[4rem] p-8 md:p-12 border border-zinc-100 shadow-2xl relative overflow-hidden group">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 md:mb-10 gap-4">
              ...
              <div className="text-left sm:text-right w-full sm:w-auto">
                <p className="text-3xl md:text-4xl font-black text-zinc-900 mb-1">{userData.xp.toLocaleString()}<span className="text-sm md:text-lg text-zinc-200 ml-2 italic">XP</span></p>
        ...
        <section className="mb-20">
          <div className="flex items-center gap-4 md:gap-6 mb-8 md:mb-12 px-2 md:px-4">
            <h2 className="text-2xl md:text-4xl font-black text-zinc-900 flex items-center gap-3 md:gap-4 tracking-tighter uppercase">
              <i className="ri-flashlight-fill text-[#C29976] text-3xl md:text-5xl"></i> Missions
            </h2>
            ...
            {activeTasks.length ? activeTasks.slice(0,6).map((task,idx) => (
              <div key={task._id} className="group p-8 md:p-10 rounded-[3rem] md:rounded-[4rem] border-2 transition-all duration-500 bg-white border-zinc-100 hover:border-[#C29976]/20 hover:shadow-xl sm:hover:-translate-y-2">
                <div className="flex justify-between items-center mb-6 md:mb-10">
                    ...
                    <div className="px-3 md:px-5 py-1.5 md:py-2 rounded-full bg-zinc-50 text-[8px] md:text-[10px] font-black text-zinc-400 uppercase tracking-widest border border-zinc-100">Pending</div>
                </div>
                <h4 className="font-black text-xl md:text-3xl mb-3 tracking-tight text-zinc-900 text-left">{task.title}</h4>
                <p className="text-zinc-400 text-xs md:text-sm font-medium mb-8 md:mb-12 italic leading-relaxed text-left line-clamp-3">
                  "{task.description || "Mission details are classified."}"
                </p>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4 md:p-8 backdrop-blur-2xl bg-black/60">
            <div className="bg-white w-full max-w-2xl rounded-t-[3rem] sm:rounded-[4rem] md:rounded-[5rem] p-8 sm:p-12 md:p-20 shadow-2xl relative overflow-y-auto max-h-[90vh]">
              <button onClick={()=>setIsModalOpen(false)} className="absolute top-6 right-6 sm:top-10 sm:right-10 text-zinc-200 hover:text-red-500 transition-colors">
                <i className="ri-close-circle-fill text-3xl sm:text-5xl"></i>
              </button>
              ...
              <form onSubmit={handleAddTask} className="space-y-6 sm:space-y-8">
                <div className="space-y-2 text-left">
                  <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest ml-4 sm:ml-6 text-zinc-400">Objective Name</label>
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-6 sm:pt-10">
                  <button type="submit" className="flex-[2] py-5 sm:py-8 bg-[#111] text-white rounded-2xl sm:rounded-[2.5rem] font-black text-xs sm:text-sm uppercase tracking-[0.2em] sm:tracking-[0.4em] hover:bg-[#C29976] shadow-xl transition-all">Confirm Launch</button>
                  <button type="button" onClick={()=>setIsModalOpen(false)} className="flex-1 py-5 sm:py-8 bg-zinc-100 text-zinc-400 rounded-2xl sm:rounded-[2.5rem] font-black text-[10px] sm:text-xs uppercase hover:bg-zinc-200 transition-all">Abort</button>
                </div>
              </form>
            </div>
          </div>
