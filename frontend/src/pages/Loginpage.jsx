...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Kirim identifier & password sesuai backend ALSIO
      ...
      console.log(err.response?.data); // debug response backend
      const backendMsg = err.response?.data?.message || "Email/Username or password incorrect.";
    ...
    <>
      ...
      <div className="min-h-screen bg-[#FAF7F4] flex flex-col items-center justify-center p-6 antialiased" style={POP}>
        ...
        <div className="w-full max-w-[1050px] h-auto md:h-[583px] bg-white rounded-[32px] shadow-lg flex flex-col md:flex-row overflow-hidden">
          {/* Left Panel */}
          <div className="hidden md:flex w-full md:w-[525px] h-[300px] md:h-full bg-cover bg-center p-12 flex-col justify-end"
               style={{ backgroundImage: "url('/images/Left_Panel.png')" }}>
            <h1 className="text-5xl text-white font-bold mb-4">Welcome.</h1>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div className="flex justify-between items-center py-1">
                <label className="flex items-center cursor-pointer">
                  ...
                  <span className="ml-2 text-xs text-gray-900 font-bold">Remember Me</span>
                </label>
                <Link to="/forgot-password" className="text-xs text-[#946C44] hover:text-[#7A5836] font-bold">
                  Forgot Password?
                </Link>
              </div>
