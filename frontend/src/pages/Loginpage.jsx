            ...
            <form onSubmit={handleLogin} className="space-y-4 sm:space-y-5">
              ...
              {(errorMsg || successMsg) && (
                <div className={`text-[10px] sm:text-xs p-2.5 rounded-lg border animate-in fade-in duration-300 ${
                  errorMsg ? 'bg-red-50 text-red-500 border-red-100' : 'bg-amber-50 text-[#946C44] border-amber-100'
                }`}>
                  {errorMsg || successMsg}
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full h-[44px] sm:h-[48px] rounded-xl sm:rounded-2xl uppercase flex items-center justify-center gap-3 text-white shadow-lg transition-all transform active:scale-[0.98] overflow-hidden hover:opacity-90 mt-2 disabled:opacity-70"
                style={{
                  ...PJS,
                  fontWeight: 800,
                  fontSize: "12px",
