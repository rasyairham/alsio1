...
const RegisterPage = () => {
  ...
  const handleRequestOTP = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.message?.toLowerCase().includes("email")) {
        ...
        showNotification(err.response?.data?.message || "Failed to send verification code.");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleFinalRegister = async (e) => {
    ...
    try {
      ...
      showNotification("Registration successful! Welcome to ALSIO.");
      navigate('/login');
    } catch (err) {
      showNotification(err.response?.data?.message || "Incorrect or expired OTP code.");
    } finally {
      setIsLoading(false);
    }
  };
